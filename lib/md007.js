...
module.exports = {
  ...
  "function": function MD007(params, onError) {
    ...
    // start_indent (number; ignored if start_indented is false, else defaults to indent)
    const startIndent = startIndented ? Number(params.config.start_indent || indent) : 0;
    flattenedLists().forEach((list) => {
      if (list.unordered && list.parentsUnordered) {
        list.items.forEach((item) => {
          const { lineNumber, line } = item;
          const expectedNesting = list.nesting + (startIndented ? 1 : 0);
          const expectedIndent = startIndent + expectedNesting * indent;
