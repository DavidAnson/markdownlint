...
const escapeParentheses = (unescaped) => unescaped.replace(/\)/g, '\\)')
...
module.exports = {
  ...
  "function": function MD054({ parsers, config }, onError) {
    let style = String(config.style || "mixed");
    ...
    for (const link of links) {
      ...
      if (
        (style === "autolink_only" && !autolink) ||
        (style === "inline_only" && (!inlineLink || autolink)) ||
