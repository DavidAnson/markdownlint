...
/**
 * Resolve referenced "extends" path ina configuration file
 * using path.resolve() and require.resolve() as a fallback.
 *
 * @param {string} configFile Configuration file name.
 * @param {*} referenceFile Referenced path to resolve.
 * @returns {string}
 */
function resolveConfigExtends(configFile, referenceFile) {
  const resolvedExtendsFile = path.resolve(
  ...
  try {
    return require.resolve(path.dirname(configFile), referenceFile);
  // eslint-disable-next-line unicorn/prefer-optional-catch-binding
  } catch (error) {
...
function readConfig(file, parsers, callback) {
  ...
  fs.readFile(file, helpers.utf8Encoding, (err, content) => {
    ...
    if (config.extends) {
      const resolvedExtends = resolveConfigExtends(file, config.extends);
