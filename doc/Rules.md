...
that are automatically generated by GitHub for headings in the document:
...
heading algorithm][github-heading-algorithm] which (among other things) converts
letters to lowercase. Therefore, the following is reported as a violation:
...
Some platforms allow the syntax `{#named-anchor}` to be used within a heading to
provide a specific name (consisting of only lower-case letters, numbers, `-`,
and `_`); that is allowed by this rule:
...
Using an `a` tag can be useful in scenarios where a heading is not appropriate
or for control over the text of the fragment identifier.
...
This rule supports the fragment syntax used by GitHub to highlight [specific
lines/sections of a document][github-linking-to-content]. For example, this link
to line 20:
...
And this link to content starting within line 19 running partly through line 21:
...
`ignored_pattern` [regular expression][RegEx] parameter to a suitable pattern
(e.g., `^figure-`).
...
5. Append dash+incrementing integer if needed for uniqueness
6. [URI-encode][encodeURIComponent] the result
...
Style `compact` avoids extra padding with a single space before and after cell
content (or just a single space for cells without content):
...
Style `tight` uses no padding at all (for cells with or without content):
